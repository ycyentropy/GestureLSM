...
def prepare_all():
    ...
    parser.add_argument(
        "--config", type=str, default="./configs/interactive_gesture_lsm.yaml"
    )
    parser.add_argument(
...
def main_worker(rank, world_size, cfg, args):
    ...
    logger.info(f"Initializing InteractiveGestureTrainer on GPU {rank}...")
    cfg.rank = rank
    trainer = __import__(
        f"trainer.interactive_gesture_trainer", fromlist=["something"]
    ).InteractiveGestureTrainer(cfg, args)
    logger.info(f"InteractiveGestureTrainer initialized successfully on GPU {rank}")
        ...
        for epoch in range(resume_epoch, cfg.solver.epochs + 1):
            ...
            if (epoch) % cfg.val_period == 0:
                ...
                if cfg.ddp:
                    ...
                    early_stop_signal = torch.tensor([should_stop], dtype=torch.int, device=trainer.model.device if hasattr(trainer.model, 'device') else f"cuda:{rank}")
