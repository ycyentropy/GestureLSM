...
def parse_args():
    """解析命令行参数"""
    parser = argparse.ArgumentParser(description='使用显存缓存的RVQ训练')
    ...
    parser.add_argument('--exp_name', type=str, default='rvq_gpu_cache', help='实验名称')
...
def main():
    ...
    # 使用分布式数据并行（如果是分布式模式）
    if is_distributed:
        model = DDP(model, device_ids=[local_rank], find_unused_parameters=False, output_device=local_rank)
