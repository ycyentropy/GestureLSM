...
def test_seamless_dataset():
    ...
    try:
        ...
        dataset = SeamlessInteractionDataset(
            data_path="datasets/seamless_interaction/improvised/train",
            split="train",
            window_size=2.0,
            window_stride=0.5,
            sample_rate=16000,
        ...
        if len(dataset) > 0:
            ...
            if 'pose' in sample:
                ...
                if isinstance(pose, torch.Tensor):
                    pose_np = pose.numpy()
                    print(f"步骤7: 姿态数据范围: [{pose_np.min():.4f}, {pose_np.max():.4f}]")
                    print(f"步骤8: 姿态数据均值: {pose_np.mean():.4f}")
                    print(f"步骤9: 姿态数据标准差: {pose_np.std():.4f}")
            ...
            # 测试窗口功能
            print("步骤11: 测试窗口功能...")
            windows = dataset.get_windows(0)
            print(f"步骤12: 第一个样本有 {len(windows)} 个窗口")
            
            if len(windows) > 0:
                window = windows[0]
                if 'pose' in window:
                    print(f"步骤13: 第一个窗口的姿态形状: {window['pose'].shape}")
